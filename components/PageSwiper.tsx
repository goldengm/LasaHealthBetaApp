...
export function mergeExtraData(...newArgs: unknown[]): number {
...
const isAndroidRTL = I18nManager.isRTL && getOS() === 'android';
...
type Props<ItemT> = {
  ...
  dotStyle: ViewStyle;
  swiperContainerStyle: ViewStyle;
  customPaginationContainerStyle: ViewStyle;
...
> {
  ...
  setActiveIndex = (activeIndexP: number) => {
    const {onChangeIndex} = this.props;
    this.setState({activeIndex: activeIndexP});
    onChangeIndex && onChangeIndex(activeIndexP);
  };

  goToSlide = (pageNum: number, triggerOnSlideChange?: boolean) => {
    const prevNum = this.state.activeIndex;
    this.setActiveIndex(pageNum);
  ...
  _renderOuterButton = (
    ...
    return (
      <View style={!this.props.bottomButton && style}>
        <TouchableOpacity
          onPress={onPress}
          customDebounceLength={this.props.customDebounceLength}
    ...
    return (
      <View
        style={{
          ...
          ...this.props.customPaginationContainerStyle,
        }}>
        <SafeAreaView>
  ...
  render() {
    ...
    return (
      <View style={{...styles.flexOne, ...this.props.swiperContainerStyle}}>
        <FlatList
